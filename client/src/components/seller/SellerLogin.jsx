...
const SellerLogin = () => {
    ...
    useEffect(() => {
        // Only redirect if seller is authenticated
        if (isSeller) {
            navigate("/seller", { replace: true });
        }
    }, [isSeller, navigate]);

    // Don't render if seller is authenticated
    if (isSeller) {
        return null;
    }
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-screen flex items-center text-sm text-gray-600'>
        ...
        <div className='flex flex-col gap-5 m-auto items-start p-8 py-12 min-w-80 sm:min-w-88 rounded-lg shadow-xl border border-gray-200'>
            ...
            <button 
                className='bg-primary text-white rounded w-full p-2 mt-4 hover:bg-primary/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed' 
                type='submit'
                disabled={isLoading}
            >
